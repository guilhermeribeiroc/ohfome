    ...
    <section className="of-panel overflow-hidden">
      ...
      <ol className="divide-y divide-cream-200">
        {[
          ...
          { icon: ShieldCheck, titulo: "Autorize a impressão direta", descricao: "Execute o instalador correspondente como administrador. Ele instala somente o certificado raiz público do OhFome; a chave privada nunca vai para o computador do cliente.", links: [{ href: "/qz/instalar-qz-ohfome-macos.command", label: "Instalador macOS", download: true }, { href: "/qz/instalar-qz-ohfome-windows.bat", label: "Instalador Windows", download: true }, { href: "/ohfome-qz-root-ca.crt", label: "Certificado público", download: true }] },
        ...
        ].map(({ icon: Icone, titulo, descricao, link, links }, indice) => <li key={titulo} className="flex gap-4 px-5 py-5 sm:px-6"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-coral-050 text-coral-600 ring-1 ring-coral-100"><Icone size={17} /></span><div><p className="text-sm font-semibold text-ink-900"><span className="mr-2 text-ink-400">0{indice + 1}</span>{titulo}</p><p className="mt-1 text-xs leading-5 text-ink-500">{descricao}</p>{link && <a href={link.href} target={"download" in link && link.download ? undefined : "_blank"} rel={"download" in link && link.download ? undefined : "noreferrer"} download={"download" in link ? link.download : undefined} className="mt-2 inline-flex text-xs font-semibold text-coral-600 hover:text-coral-700">{link.label} <ChevronRight size={14} /></a>}{links && <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1">{links.map((item) => <a key={item.href} href={item.href} download className="inline-flex text-xs font-semibold text-coral-600 hover:text-coral-700">{item.label} <ChevronRight size={14} /></a>)}</div>}</div></li>)}
      </ol>
    </section>
  </div>;
}
