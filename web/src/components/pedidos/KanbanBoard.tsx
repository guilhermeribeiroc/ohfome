...
const TIPO_LABEL: Record<Pedido["tipo"], string> = { mesa: "Mesa", balcao: "Controle de pedidos", delivery: "Delivery" };
...
export function KanbanBoard({ titulo = "Painel de pedidos", subtitulo = "Controle de pedidos em tempo real", colunas = TODAS_COLUNAS, permiteCriar = true, permiteCancelar = true, origem, modoCozinha = false }: KanbanBoardProps) {
...
function NovoPedidoModal({ onFechar, onCriado }: { onFechar: () => void; onCriado: () => void }) {
  ...
  return <div className="of-modal-backdrop" role="dialog" aria-modal="true" aria-label="Novo pedido"><div className="of-modal-panel flex h-[min(90dvh,720px)] max-w-lg flex-col">
    <header className="of-panel-header bg-surface"><div><p className="of-eyebrow">Controle de pedidos</p><h2 className="font-display text-lg font-bold tracking-tight">Novo pedido</h2></div><button onClick={onFechar} className="of-icon-btn" aria-label="Fechar"><X size={17} /></button></header>
