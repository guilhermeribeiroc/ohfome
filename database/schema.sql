...
create table pedidos (
  ...
  -- Preenchido quando este pedido e um adicional lancado pelo cliente sobre
  -- um pedido ja enviado (cardapio digital). Ver fn_adicionar_itens_pedido_publico.
  pedido_pai_id uuid references pedidos(id) on delete set null,
  observacoes text,
...
create index idx_pedidos_pedido_pai_id on pedidos(pedido_pai_id) where pedido_pai_id is not null;
  ...
  select jsonb_build_object(
    ...
    'adicionais', coalesce(
      (select jsonb_agg(jsonb_build_object(
          ...
          'itens', coalesce(
            (select jsonb_agg(jsonb_build_object('produtoNome', pr2.nome || case when ip2.tamanho is not null then ' (' || ip2.tamanho::text || ')' else '' end, 'quantidade', ip2.quantidade) order by ip2.created_at)
...
-- Adiciona itens a um pedido ja enviado (cliente do cardapio digital pediu
-- algo a mais). Vira um pedido novo e independente, vinculado via
-- pedido_pai_id, para que a impressao na cozinha traga so o que foi
-- adicionado — mesmo padrao das "rodadas" do Garcom/Mesa.
create function fn_adicionar_itens_pedido_publico(
  p_slug text,
  p_pedido_id uuid,
  p_itens jsonb,
  p_observacoes text
